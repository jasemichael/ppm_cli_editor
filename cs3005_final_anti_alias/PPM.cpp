...
void PPM::antiAlias( const int& n, PPM& dst ) const{
    int redTotal, greenTotal, blueTotal, amountPix, i, row, column;
    redTotal = 0;
    greenTotal = 0;
    blueTotal = 0;
    amountPix = n*n;
    for(i=0; i < (getWidth()*getHeight()/n); i++){
        for(row=0; row < n;row++){
            for(column=0; column < n; column++){
                redTotal += getChannel(row, column, 0);
                greenTotal += getChannel(row, column, 1);
                blueTotal += getChannel(row, column, 2);
            }
        }
    }
    dst = PPM(getHeight()/n, getWidth()/n);
    for(row=0; row < dst.getHeight(); row++){
        for(column=0; column < dst.getWidth(); column++){
