...
void showMenu(std::ostream& os){
    ...
    << "anti-alias) Set output image from Anti-alias filter on input image 1." << "set-color-gradient) Smoothly set the RGB values for a range of slots in the color table." <<"\nquit) Quit" << "\n\n";
}
...
void takeAction(std::istream& is, std::ostream& os, const std::string& choice, PPM& input_image1, PPM& input_image2, PPM& output_image, NumberGrid& grid, ColorTable& table){
    ...
    else if(choice == "anti-alias"){
        antiAliasFilter(is, os, input_image1, output_image);
    }
    else{
...
void antiAliasFilter( std::istream& is, std::ostream& os, const PPM& src, PPM& dst ){
    int n = getInteger(is, os, "Reduction count? ");
    src.antiAlias(n, dst);
}