    ...
    if(colors.getNumberOfColors() >= 3){
        ...
        for(row = 0; row < ppm.getHeight(); row++){
            for(column = 0; column < ppm.getWidth(); column++){
                ...
                else if(getNumber(row, column) == 0){
                    ppm.setPixel(row, column, colors[colors.getNumberOfColors()-2].getRed(), colors[colors.getNumberOfColors()-2].getGreen(), colors[colors.getNumberOfColors()-2].getBlue());
                }
                else{
                    ppm.setPixel(row, column, colors[getNumber(row, column) % (colors.getNumberOfColors()-2)].getRed(), colors[getNumber(row, column) % (colors.getNumberOfColors()-2)].getGreen(), colors[getNumber(row, column) % (colors.getNumberOfColors()-2)].getBlue());
                }
            }
        }
    }
